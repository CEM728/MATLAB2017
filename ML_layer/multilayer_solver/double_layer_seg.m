function [A, An] = double_layer_seg(k, t, s, isinterface, a) 
if nargin < 5, 
    a = 0; 
end
wantder = nargout>1;
...
%     N = numel(s_copy.x); 
%     M = numel(s_copy.x);                                              % number of source=target pts
...
%     opt.Dker_noang = (1i*k/4) * opt.Dker_noang;
    ...
    B  = layerpot.D(k, s_copy, [],opt);  
    if wantder
        Bn = layerpot.T(k, s_copy, [],opt);  
    end
else
    %for fast hankel function (hank106)
%     N = numel(s_copy.x); 
%     M = numel(t.x);                                               % number of target pts
...
%     opt.Dker_noang = (1i*k/4) * opt.Dker_noang;
    ...
    B  = layerpot.D(k, s_copy, t,opt);  
    if wantder, 
        Bn = layerpot.T(k, s_copy, t,opt);  
    end
end
