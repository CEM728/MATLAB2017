function old = TanhSinhQuad_2deg_lf(a, b, tol)
...
s = eta * Somm_2deg_lf(gamma,0); % Call function to calculate first term
[n,s] = TruncIndex_2deg_lf(a,b,eh, kappa, nmax, s, eta);
old = sigma * h * s;
for m  = 1 : maxlev
    e2h = eh;
    h = h/2;
    eh = exp(h);
    s =  PartSum_2deg_lf(a,b,eh, e2h, n, eta);
    val = old/2 + sigma * h * s;
