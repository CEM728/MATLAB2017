...
% f = .2e12; % good conductivity
% f = .7e12; % good conductivity
% f = .8362e12; % good conductivity
...
for i = 1 : length(p)
    ...
    if nu == 0 % TE case
        ...
        h = 1;
        val_1(i) = TanhSinhQuad_2deg_lf(0, k1 + .01i, tol); % Integrate upto k through DE
        h = 1;
        val_2(i) = TanhSinhQuad_2deg_lf(k1 + .01i, a, tol); % Integrate k upto a through DE
        h = 1;
        val_3(i) = PE_Levin_2deg_lf(a, tol, q); % Tail through PE Levin with Lucas
    else
        h = 1;
        val_1(i) = TanhSinhQuad_2deg_lf(.01i, k1, tol); % Integrate upto k through DE
        h = 1;
        val_2(i) = TanhSinhQuad_2deg_lf(k1, a, tol); % Integrate k upto a through DE
        h = 1;
        val_3(i) = PE_Levin_2deg_lf(a, tol, q); % Tail through PE Levin with Lucas
    end
...
hold on
h6 = loglog(p*k1, abs(exp(-1i*k1*p)./(4*pi*p))./k1, 'linewidth',1.0);
